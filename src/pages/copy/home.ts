...
const es = {
  ...
  lead: "RPGRoll agrega una capa de rol completa sobre un servidor Paper vanilla: cada jugador tiene una raza, una clase, seis atributos al estilo D&D, un pool de salud y maná independiente de los corazones de Minecraft, habilidades con costo de maná y cooldown, trabajos con recompensas, y un sistema de progresión que desbloquea contenido automáticamente al subir de nivel. El núcleo vive en un solo plugin ({jar}); cada sistema adicional es un addon independiente que se instala aparte.",
  ...
  addonsLead:
    "Cada addon es un plugin separado que depende del núcleo ({depend}) y se instala dejando su propio jar en {dir}. Ninguno es obligatorio — instalá solo los que necesite tu servidor.",

  archTitle: "Arquitectura en breve",
  archBody:
    "El núcleo se organiza en tres módulos Gradle ({api} pública para addons, {common} utilidades compartidas, {core} la implementación real) y cada addon repite ese mismo patrón de soft-dependencia: comprueba con {check} antes de tocar cualquier clase de otro addon, así ninguno rompe si el otro no está instalado.",
  ...
  step1: "Compilá o descargá {jar} (núcleo) y colocalo en {dir} de tu servidor Paper.",
...
const en: HomeCopy = {
  ...
  step1: "Build or download {jar} (the core) and drop it into {dir} on your Paper server.",
...
const pt: HomeCopy = {
  ...
  addonsLead:
    "Cada addon é um plugin separado que depende do núcleo ({depend}) e se instala colocando o seu próprio jar em {dir}. Nenhum é obrigatório — instale só o que o seu servidor precisa.",

  archTitle: "Arquitetura em resumo",
  archBody:
    "O núcleo se organiza em três módulos Gradle ({api} pública para addons, {common} utilitários compartilhados, {core} a implementação real) e cada addon repete o mesmo padrão de soft-dependency: verifica com {check} antes de tocar qualquer classe de outro addon, então nada quebra se o outro não estiver instalado.",
  ...
  step1: "Compile ou baixe {jar} (o núcleo) e coloque-o em {dir} do seu servidor Paper.",
