    ...
    <>
      ...
      <Table>
        ...
        <tbody>
          ...
          <Tr><Td>Subida remota genérica</Td><Td>PUT/POST autenticado genérico (header configurable) a cualquier endpoint propio, proxy, o servicio compatible.</Td></Tr>
          <Tr><Td>Subida a S3 (firmada)</Td><Td>PUT firmado con AWS Signature V4 real, directo a un bucket S3 o cualquier endpoint S3-compatible (MinIO, R2, B2...) — sin proxy intermedio.</Td></Tr>
      ...
      <p>
        <code>remote-upload.mode</code> elige entre <code>generic</code> (el PUT/POST de siempre) y{" "}
        <code>s3</code>. En modo <code>s3</code>, <code>S3UploadSettings</code> configura endpoint/bucket/región/
        credenciales y si el estilo es virtual-hosted (<code>bucket.endpoint/key</code>, el default de AWS) o
        path-style (<code>endpoint/bucket/key</code>, lo que piden MinIO y varios proveedores no-AWS).
      </p>
      <CodeBlock
        language="java"
      ...
      />
      <Callout tone="warning" title="Implementado según spec, no probado contra un bucket real">
        <code>AwsSignatureV4</code> sigue al pie de la letra el algoritmo documentado por AWS (canonical request,
        string-to-sign, clave de firma derivada, firma final), usando solo <code>javax.crypto</code>/
        <code>java.security</code> del JDK — sin el SDK de AWS. Pero no hay credenciales de AWS disponibles en el
        entorno donde se escribió esto: probalo una vez contra tu bucket/endpoint real antes de confiar en esto en
        producción.
      </Callout>
      <p>
      ...
      <p>
        Además de <code>assets/</code> (cliente), cada módulo puede traer su propia carpeta{" "}
        <code>data/&lt;namespace&gt;/...</code> con tags, loot tables, recipes, advancements y functions.{" "}
        <code>DatapackMergeEngine</code> vive separado de <code>MergeEngine</code> a propósito — la estrategia de
        fusión es distinta: los archivos bajo <code>tags/</code> se fusionan de verdad (unión del array{" "}
        <code>values</code>, sin duplicados), pero el resto (loot tables, recipes, advancements, functions) se
        copia tal cual y el último módulo pisa, porque no tiene sentido "fusionar" dos recetas distintas con el
        mismo id.
      </p>
      <Callout tone="info" title="Un datapack no es un resource pack — vive server-side">
        Un resource pack se manda al cliente vía <code>Player#setResourcePack</code>. Un datapack (recetas, loot
        tables, advancements, functions) es puramente server-side: vive en{" "}
        <code>&lt;mundo primario&gt;/datapacks/&lt;nombre&gt;/</code> y nunca se envía a nadie. Por eso{" "}
        <code>DatapackBuildEngine</code> no empaqueta ni sirve nada por HTTP — solo fusiona y copia la carpeta.
      </Callout>
      <p>
        Nada se aplica solo: <Kbd>/srp datapack build</Kbd> fusiona <code>data/</code> de todos los módulos y
        copia el resultado a <code>datapacks/</code>, y por separado <Kbd>/srp datapack reload</Kbd> llama a{" "}
        <code>Server#reloadData()</code> — el método real de Paper para recargar recetas/loot tables/tags/
        advancements sin reiniciar plugins (verificado contra el jar real de la API, distinto de un{" "}
        <code>/reload</code> completo). Deshabilitado por defecto (<code>datapack.enabled: false</code>);{" "}
        <code>datapack.pack-format</code> es una clave separada de <code>pack-format</code> porque datapack y
        resource pack no comparten la misma numeración de <code>pack_format</code> entre versiones de Minecraft.
      </p>
      ...
      <p>
        Las texturas <code>.png</code> se muestran como una cabeza de jugador con esa imagen "pintada" encima —
        el host HTTP embebido gana un segundo contexto (<code>/assets/</code>) que sirve cualquier archivo suelto
        del árbol fusionado (con protección contra path traversal), y{" "}
        <code>ItemBuilder.skullFromUrl(...)</code> arma un <code>PlayerProfile</code> con el payload de textura
        apuntando a esa URL — el mismo truco que usa <code>ItemBuilder.skull(...)</code> de :core, pero armando
        el JSON nosotros mismos en vez de partir de un base64 ya hecho.
      </p>
      <Callout tone="warning" title="Preview parcial, no un render real — y sin probar contra un cliente real">
        Solo funciona con texturas (no con modelos/sonidos/fuentes), requiere que el cliente del jugador pueda
        alcanzar el host HTTP del plugin por red, y la imagen se distorsiona al envolverse sobre el UV de una
        cabeza en vez de mostrarse plana. Si el pack se sirve desde una <code>http.public-url</code> externa en
        vez del host embebido, no hay preview — el explorador cae de vuelta al ícono+lore de siempre. Tampoco se
        pudo verificar contra un cliente de Minecraft real en el entorno donde se escribió esto.
      </Callout>
      ...
      <Table>
        ...
        <tbody>
          ...
          <Tr><Td className="font-mono text-xs">/srp datapack build</Td><Td>Fusiona <code>data/</code> de todos los módulos y lo copia a <code>datapacks/</code> (requiere <code>datapack.enabled: true</code>).</Td></Tr>
          <Tr><Td className="font-mono text-xs">/srp datapack reload</Td><Td>Llama a <code>Server#reloadData()</code> para aplicar el datapack ya copiado.</Td></Tr>
        </tbody>
      </Table>
      <p>
      ...
      <Callout tone="tip" title="Scope completo, y ya cerrado del todo">
        SackResourcePack es el único addon de esta serie construido a "todo de una" en vez del tier recomendado, y
        los tres huecos que quedaban deliberadamente afuera de esa primera pasada (firma AWS S3 SigV4, fusión de{" "}
        <code>data/</code> de datapack, preview visual de assets) ya están implementados. Lo que queda no es
        scope recortado sino límites honestos de lo que se pudo verificar en el entorno donde se escribió esto:
        el firmado S3 nunca se probó contra un bucket real, y el preview de skull-texture nunca se probó contra
        un cliente de Minecraft real — probá ambos una vez en tu entorno antes de confiar en ellos en producción.
      </Callout>
